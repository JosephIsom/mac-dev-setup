...
install_theme() {
  ...
  find "$REPO_WARP_THEME_DIR" -type f -name 'islands-dark.yaml' -exec cp {} "$TARGET_WARP_THEME_DIR/" \;
}
...
Installed custom theme:
  $TARGET_WARP_THEME_DIR/islands-dark.yaml
...
verify_install() {
  ...
  [[ -f "$TARGET_WARP_THEME_DIR/islands-dark.yaml" ]] || die "Warp theme not found after installation."
  [[ -f "$TARGET_WARP_NOTES" ]] || die "Warp bootstrap notes not found at $TARGET_WARP_NOTES after installation."
  grep -Fq 'name: Islands Dark' "$TARGET_WARP_THEME_DIR/islands-dark.yaml" || die "Warp theme file is missing the managed theme name."
