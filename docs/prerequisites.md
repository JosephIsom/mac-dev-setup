...
| --- | --- | --- | --- | --- | --- |
| `scripts/prerequisites/xcode/xcode-clt.sh` | Installs Xcode Command Line Tools. | Many Homebrew packages, compilers, and developer workflows depend on Apple’s CLI toolchain. The repo’s C/C++ baseline also depends on it for Apple `clang`, `clang++`, `lldb`, SDKs, and system headers. | You usually do not use this directly after setup; it is a foundation for compilers and Apple SDK headers. | [Xcode Command Line Tools](https://developer.apple.com/xcode/resources/) | None unless macOS prompts for extra license acceptance. |
