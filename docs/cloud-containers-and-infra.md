...
| `cloud/aws/aws-config.sh` | On | AWS config support. | Use `~/.aws/config` as your main CLI profile file. | [AWS CLI config](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html) | Add your real profiles after bootstrap. |
...
| `containers/caddy/caddy.sh` | On | Installs host-native Caddy for optional local edge and reverse proxy work. | Use the binary directly, or pair it with the dedicated [`local-edge/`](../local-edge) flow. | [Caddy](https://caddyserver.com/) | Bootstrap only installs the Caddy tool. Local edge provisioning still requires `./scripts/bootstrap-local-edge.sh setup`. |
